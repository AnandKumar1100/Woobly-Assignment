...
const RestaurantDetails = ({ scaleImage, animateImage, getProgressStyles, toggleReadMoreStatus, shouldShowReadMore, currentIndex})=> {
    return(
        <Container>
        ...
        <View style={{ flex: 1, position: "absolute", zIndex: -100 }}>
          <Swiper style={{ height: deviceHeight / 1.07, width: deviceWidth }} onIndexChanged={animateImage} showsPagination={true}
           autoplayTimeout={4} autoplay={true} paginationStyle={{position:"absolute", paddingBottom:deviceWidth/3}} dot={
             <View style={{backgroundColor:'grey', width: (deviceWidth/constants.photos.length)-70/constants.photos.length, height: 1, marginLeft: 3, marginRight: 3}} />
           }
           activeDot={
             <View style={{backgroundColor:'white', width: (deviceWidth/constants.photos.length)-70/constants.photos.length, height: 1, marginLeft: 3, marginRight: 3}} />
           }>
        ...
        <View style={{flex:1, marginTop: deviceHeight/2.5}}>
            <View style={{paddingLeft:25, paddingRight:25}}>
              ...
              <View style={{ marginTop: 25, flexDirection:"row" }}>
                ...
                /> */}
                {constants.photos.map((eachPhoto, index)=><Animated.View key={index} style={[ { marginRight:5, backgroundColor: index == currentIndex ? "white" : "grey", height:1, width:(deviceWidth/constants.photos.length)-70/constants.photos.length}]}/>)}
              </View>
              </View>
